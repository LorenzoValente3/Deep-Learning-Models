...
Implementation of _Variational Autoencoder_ with factorized gaussian posteriors, $` q_{\phi}(z|x ) = \mathcal{N}(z, \mu(x),diag(\sigma^{2})) `$ and standard normal latent variables $`p(z) =\mathcal{N}(0, \pmb I ) `$.  $`\sqrt{2}`$
